...
int main() {
	...
	t3.loadFromFile("images/neon_ball.png");
	...
	while (window.isOpen())
	{
		...
		if (ball_pos.x < 0 || ball_pos.x > width - 25) dx = -dx;
		if (ball_pos.y < 0 || ball_pos.y > height - 25) dy = -dy;
		...
		// reflect on the paddle
		if (FloatRect(ball_pos.x, ball_pos.y, 25, 25).intersects(paddle.getGlobalBounds()))
			dy = -(rand() % 5 + 2);
